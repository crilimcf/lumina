...
const ROOM_SELECT = `
  SELECT r.id, r.name, r.topic, r.description, r.image_url, r.visibility,
         ...
         (SELECT count(*) FROM room_members rm JOIN users member ON member.id=rm.user_id AND member.suspended_at IS NULL WHERE rm.room_id = r.id)::int AS member_count,
  ...
  FROM rooms r JOIN users u ON u.id = r.creator_id AND u.suspended_at IS NULL
`;
...
async function blocked(a,b,query=q){
  ...
  const {rows}=await query(`SELECT 1 FROM blocks WHERE (blocker_id=$1 AND blocked_id=$2) OR (blocker_id=$2 AND blocked_id=$1) LIMIT 1`,[a,b]);
  return !!rows[0];
}
...
async function assertDiscoverable(roomId,userId){const room=await getRoom(roomId,userId);if(!canDiscover(room,userId)||await blocked(userId,room.creator_id))throw notFound('Sala não encontrada');return room;}
async function assertMember(roomId,userId){const room=await getRoom(roomId,userId);if(!room||!room.joined||room.billing_state!=='active'||await blocked(userId,room.creator_id))throw forbidden('Não tens acesso a esta sala');return room;}
