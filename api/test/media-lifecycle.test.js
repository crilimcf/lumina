...
async function confirmedUpload(ownerId, suffix) {
  ...
  await q(`INSERT INTO uploads (owner_id, key, url, mime, bytes, confirmed_at) VALUES ($1,$2,$3,'image/jpeg',1234,now())`, [ownerId, key, url]);
  return url;
}

async function communityFor(user, slug) {
  const out = await request('/communities', { method: 'POST', token: user.token, body: { slug, name: slug, seedProposals: ['primeiro teste','segundo teste','terceiro teste','quarto teste','quinto teste'] } });
  assert.equal(out.response.status, 201, JSON.stringify(out.data));
  return out.data;
}
...
test('um upload confirmado só pode ser consumido por um conteúdo', async () => {
  ...
  const post = await request('/posts', { method: 'POST', token: alice.token, body: { communityId: community.id, body: 'usa a imagem uma vez', mediaUrl: url, palette: 0 } });
  ...
  assert.ok(rows[0].consumed_at); assert.equal(rows[0].purpose, 'post');
  const reuse = await request('/moments', { method: 'POST', token: alice.token, body: { mediaUrl: url, palette: 0 } });
  assert.equal(reuse.response.status, 400); assert.equal(reuse.data.code, 'unconfirmed_upload');
});

test('apagar o original apaga também o repost e liberta o upload', async () => {
  const alice = await register('repost.alice'); const bob = await register('repost.bob');
  ...
  const post = await request('/posts', { method: 'POST', token: alice.token, body: { communityId: community.id, body: 'imagem original', mediaUrl: url, palette: 0 } });
  assert.equal(post.response.status, 201, JSON.stringify(post.data));
  ...
  const deleted = await request(`/posts/${post.data.id}`, { method: 'DELETE', token: alice.token }); assert.equal(deleted.response.status, 200);
...
test('foto de uma vez é purgada da mensagem e da tabela de uploads quando expira', async () => {
  const alice = await register('once.alice'); const bob = await register('once.bob');
  ...
  const sent = await request(`/messages/threads/${thread.data.id}/messages`, { method: 'POST', token: alice.token, body: { kind: 'media', mode: 'once', mediaUrl: url, palette: 0 } });
  assert.equal(sent.response.status, 201, JSON.stringify(sent.data));
  const opened = await request(`/messages/${sent.data.id}/open`, { method: 'POST', token: bob.token }); assert.equal(opened.response.status, 200, JSON.stringify(opened.data));
  await q(`UPDATE messages SET expires_at=now()-interval '1 second' WHERE id=$1`, [sent.data.id]); assert.equal(await purgeMessages(), 1);
  const msg = await q('SELECT body,media_url,purged_at FROM messages WHERE id=$1', [sent.data.id]);
  assert.equal(msg.rows[0].body, null); assert.equal(msg.rows[0].media_url, null); assert.ok(msg.rows[0].purged_at);
  assert.equal((await q('SELECT 1 FROM uploads WHERE url=$1', [url])).rows.length, 0);
});
...
test('Momento publicado pode substituir media sem reiniciar 24h e sem deixar upload antigo', async () => {
  ...
  const forbiddenEdit = await request(`/moments/${created.data.id}`, { method: 'PATCH', token: bob.token, body: { palette: 4 } });
  assert.equal(forbiddenEdit.response.status, 404);
  const edited = await request(`/moments/${created.data.id}`, { method: 'PATCH', token: alice.token, body: { mediaUrl: replacementUrl, palette: 3 } });
  assert.equal(edited.response.status, 200, JSON.stringify(edited.data));
  assert.equal(edited.data.media_url, replacementUrl); assert.equal(edited.data.palette, 3);
  ...
  assert.ok(replacement.rows[0].consumed_at); assert.equal(replacement.rows[0].purpose, 'moment');
});

test('Momento expirado remove a linha e o upload associado', async () => {
  const alice = await register('moment.alice'); const url = await confirmedUpload(alice.user.id, 'moment-photo');
  ...
  await q(`UPDATE moments SET expires_at=now()-interval '1 second' WHERE id=$1`, [moment.data.id]); assert.equal(await purgeMoments(), 1);
...
test('apagamento RGPD do autor remove reposts derivados e o upload', async () => {
  const alice = await register('erase.alice'); const bob = await register('erase.bob');
  const community = await communityFor(alice, 'erase-media'); await request(`/communities/${community.id}/join`, { method: 'POST', token: bob.token });
  const url = await confirmedUpload(alice.user.id, 'erase-source');
  const post = await request('/posts', { method: 'POST', token: alice.token, body: { communityId: community.id, body: 'vai ser apagado', mediaUrl: url, palette: 0 } });
  assert.equal(post.response.status, 201, JSON.stringify(post.data));
  const repost = await request(`/posts/${post.data.id}/repost`, { method: 'POST', token: bob.token }); assert.equal(repost.response.status, 201, JSON.stringify(repost.data));
  await request('/account/delete', { method: 'POST', token: alice.token });
  await q(`UPDATE deletion_requests SET execute_at=now()-interval '1 minute' WHERE user_id=$1`, [alice.user.id]); assert.equal(await runAccountDeletions(), 1);
  assert.equal((await q('SELECT 1 FROM posts WHERE id=$1', [repost.data.id])).rows.length, 0);
  assert.equal((await q('SELECT 1 FROM uploads WHERE url=$1', [url])).rows.length, 0);
});
