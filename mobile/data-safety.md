...
| Localização precisa/aproximada | Sim, quando pedida | Não persistida como coordenadas no perfil | Opcional | Contexto local escolhido pelo utilizador |
...
- Service providers, não partilha comercial: Railway/PostgreSQL, Vercel, Cloudflare R2, Resend, Apple APNs e Google FCM; Stripe apenas se uma funcionalidade paga for efetivamente ativada.
...
| Localização | Ao ativar uma experiência local | Mostrar contexto local; não é pedida no arranque |
