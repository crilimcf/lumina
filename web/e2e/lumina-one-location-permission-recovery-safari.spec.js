...
test('pede localização real mesmo se Permissions API disser denied', async ({ page, context }) => {
  ...
  await page.route('https://nominatim.openstreetmap.org/reverse**', route => route.fulfill({
    ...
    body: JSON.stringify({ address: { city: 'Bragança', country: 'Portugal' } }),
  }));

  await register(page);
  await page.getByRole('button', { name: 'Detetar localização do iPhone' }).click();

  await expect(page.getByPlaceholder('Porto, Lisboa, Braga…')).toHaveValue('Bragança');
