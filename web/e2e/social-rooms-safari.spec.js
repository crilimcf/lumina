...
test('Atividade permite aceitar pedido e mudar perfil para privado em Mobile Safari',async({page})=>{
  ...
  await page.route('**/api/notifications',route=>route.fulfill({status:200,contentType:'application/json',body:JSON.stringify({notifications:[{
    id:notificationId,type:'follow_request',read_at:accepted?new Date().toISOString():null,created_at:new Date().toISOString(),
    follow_request_id:requestId,follow_request_status:accepted?'accepted':'pending',actor_id:actorId,actor_handle:'pessoa.privada',actor_name:'Pessoa Privada',actor_palette:1,actor_avatar_url:null,
  }],nextCursor:null})}));
  await page.route('**/api/users/me/privacy',async route=>{
    if(route.request().method()==='PATCH'){
      const body=route.request().postDataJSON();isPrivate=!!body.isPrivate;
