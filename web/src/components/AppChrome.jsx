...
export function Nav({ tab,setTab,setThread,setComp,coms,threads=[],ping }) {
  ...
  return <div className="nav" style={{display:'grid',gridTemplateColumns:'repeat(5,minmax(0,1fr))',gap:0}}>{items.map(([k,I,label,aria])=><button key={k} aria-label={aria} className={`nb${tab===k?' nb-on':''}`} style={{padding:'5px 2px',minWidth:0,position:'relative'}} onClick={()=>{
    if(k==='new') { if(!coms.length) return ping?.('O Feed ainda não está pronto para publicar nesta conta.'); setThread(null);setTab('feed');setComp({community:coms[0].id,title:'Publicar'});return; }
...
export function Composer({ comp,setComp,coms,file,setFile,body,setBody,busy,publish }) {
  ...
  return <>
    <div onClick={()=>!busy&&setComp(null)} style={{position:'fixed',inset:0,background:'rgba(24,18,60,.38)',backdropFilter:'blur(5px)',display:'flex',alignItems:'flex-end',zIndex:60}}><div onClick={e=>e.stopPropagation()} className="in" style={{background:'linear-gradient(180deg,#F7F5FF,#E9E7F8)',borderRadius:'30px 30px 0 0',width:'100%',maxWidth:560,maxHeight:'94dvh',overflowY:'auto',margin:'0 auto',padding:'22px 20px calc(26px + env(safe-area-inset-bottom))',boxShadow:'0 -18px 60px rgba(30,22,70,.16)'}}>
      <div style={{display:'flex',alignItems:'flex-start',gap:12,marginBottom:16}}><div style={{flex:1}}><h3 className="d" style={{fontSize:28,lineHeight:1,margin:0}}>{comp.title}</h3>{!comp.inviteId&&<div className="m" style={{marginTop:6}}>Partilha uma fotografia, vídeo ou pensamento no teu Feed</div>}</div><button className="p" onClick={()=>setComp(null)} aria-label="Fechar" style={{padding:10}}><X size={16}/></button></div>
