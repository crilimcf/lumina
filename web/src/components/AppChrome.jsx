...
export function TopActions({ tab,setTab,setThread,unreadCount }) {
  ...
  return <div className="top-actions" aria-label="Atalhos pessoais">
    <button className={`top-action${tab==='alerts'?' top-action-on':''}`} onClick={()=>go('alerts')} aria-label={shownUnread?`Alertas, ${shownUnread} por ler`:'Alertas'}><Bell size={18} strokeWidth={tab==='alerts'?2.5:2}/>{shownUnread>0&&<span className="top-action-badge">{shownUnread>99?'99+':shownUnread}</span>}</button>
    <button className={`top-action${tab==='me'?' top-action-on':''}`} onClick={()=>go('me')} aria-label="Perfil"><User size={18} strokeWidth={tab==='me'?2.5:2}/></button>
  </div>;
}
