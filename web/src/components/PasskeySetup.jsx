...
export function PasskeySetup() {
  ...
  return <div className="card" style={{padding:20,marginBottom:14}}>
    ...
    <button type="button" className="p p-brand" disabled={busy} style={{width:'100%',padding:14,marginTop:items.length?8:0}} onClick={async()=>{setBusy(true);setError('');setNotice('');try{await registerPasskey();await refresh();setNotice('Face ID / biometria ativado neste dispositivo.');}catch(e){setError(e.message || 'Não foi possível ativar neste dispositivo.');}finally{setBusy(false);}}}>{busy?'A ativar…':items.length?'Adicionar outro dispositivo':'Ativar Face ID / biometria'}</button>
