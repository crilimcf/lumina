...
import { groupCallCopy } from './groupCallCopy.js';
...
export function IncomingCall({ call, busy, onAccept, onDecline }) {
  ...
  const incomingLabel = call.group
  ...
  return <div role="dialog" aria-modal="true" aria-label={dialogLabel} style={{position:'fixed',inset:0,zIndex:205,background:'radial-gradient(circle at 50% 28%,#49347D,#17102E 48%,#080711 80%)',display:'grid',placeItems:'center',color:'#fff',padding:22}}>
    <div style={{width:'100%',maxWidth:360,textAlign:'center'}}>
      ...
      <div style={{marginTop:8,opacity:.72}}>{incomingLabel}</div>
      <div style={{marginTop:7,fontSize:12,opacity:.52}}>{ringingLabel}</div>
