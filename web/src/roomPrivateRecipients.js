...
export async function roomPrivateRecipients(roomId, query = '') {
  const res = await fetch(`${BASE}/rooms/${encodeURIComponent(roomId)}/private-recipients?q=${encodeURIComponent(query)}`, { credentials:'include', headers:nativeAuthHeaders() });
