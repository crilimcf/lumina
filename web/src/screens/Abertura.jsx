...
export function Abertura({ me, coms, days, onSkip, onRooms, onCreateCommunity }) {
  ...
  return (
    <div style={{ minHeight:'100dvh', position:'relative', background:'linear-gradient(180deg,#EFEDFB,#DFDCF2)' }}>
      {step>=1&&<><div className="halo" style={{top:-70,right:-60,width:240,height:240,background:'#FF9A7E'}}/><div className="halo" style={{bottom:130,left:-80,width:220,height:220,background:'#9C93F2',animationDelay:'.3s'}}/></>}
      <div style={{position:'relative',maxWidth:460,margin:'0 auto',padding:'26px 20px 40px',minHeight:'100dvh',display:'flex',flexDirection:'column'}}>
        ...
        <div style={{marginBottom:30}}>
          {step===0?<><Skeleton w="58%" h={15} st={{marginBottom:18}}/><Skeleton w="90%" h={44} st={{marginBottom:12}}/><Skeleton w="62%" h={44}/></> : coms.length===0 ? <>
            ...
            <div className="m up" style={{color:'var(--coral)',marginBottom:16}}>O teu espaço está pronto</div>
        ...
        {step>=2&&<div className="up" style={{animationDelay:'.2s',marginBottom:'auto'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(14,1fr)',gap:7}}>{days.slice(0,27).map((d,i)=><div key={d.date} className="dot" style={{aspectRatio:'1',background:d.answered?'var(--ink)':'rgba(20,18,42,.09)',transform:d.answered?'none':'scale(.62)',transitionDelay:`${i*22}ms`}}/>)}<div className="dot glow" style={{aspectRatio:'1',background:'var(--coral)'}}/></div>
        </div>}

        <div style={{marginTop:34}}>
          {step===0?<Skeleton w="100%" h={52} r={99}/>:step>=2&&(coms.length===0?<button className="p p-cr up" onClick={onCreateCommunity} style={{width:'100%',padding:15,fontSize:15,display:'flex',alignItems:'center',justifyContent:'center',gap:9}}>Criar ou entrar numa comunidade <ArrowUpRight size={17}/></button>:<><button className="p p-cr up" onClick={onSkip} style={{width:'100%',padding:15,fontSize:15,display:'flex',alignItems:'center',justifyContent:'center',gap:9}}>Ver o feed <ArrowUpRight size={17}/></button><button className="p up" onClick={onRooms} style={{width:'100%',marginTop:10,padding:'13px 16px',fontSize:15,background:'rgba(255,255,255,.45)',color:'var(--ink)',display:'flex',alignItems:'center',justifyContent:'center',gap:8}}><DoorOpen size={16}/>Explorar Salas</button></>)}
        </div>
      </div>
    </div>
  );
