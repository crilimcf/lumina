...
export function Conversas({ me, tab, setTab, comp, setComp, ping, unreadCount, threads, thread, setThread, msgs, text, setText, mode, setMode, onceFile, setOnceFile, sending, send, end }) {
  ...
  return <div style={{ minHeight: '100dvh', paddingBottom: 100, background: 'linear-gradient(180deg,#EFEDFB,#DFDCF2)' }}><div style={{ maxWidth: 460, margin: '0 auto', padding: 20 }}><div style={{ display: 'flex', alignItems: 'center', gap: 10, margin: '10px 0 26px' }}><h2 className="d" style={{ fontSize: 42, flex: 1 }}>Conver<span className="it">sas</span></h2><TopActions tab={tab} setTab={setTab} setThread={setThread} unreadCount={unreadCount} /></div>{threads.length === 0 && <Empty>Ainda sem conversas.<br />Abre o perfil de alguém para falar.</Empty>}<div style={{ display: 'grid', gap: 11 }}>{threads.map((t, i) => <button key={t.id} onClick={() => setThread({ id: t.id, name: t.name, handle: t.handle, palette: t.palette, avatar_url: t.avatar_url })} className="card in" style={{ border: 0, cursor: 'pointer', padding: 15, display: 'flex', gap: 13, alignItems: 'center', textAlign: 'left', animationDelay: `${i * 60}ms` }}><Orb p={t.palette} avatarUrl={t.avatar_url} s={44} /><span style={{ flex: 1, minWidth: 0 }}><span style={{ display: 'block', fontSize: 15, fontWeight: 600 }}>{t.name}</span><span style={{ display: 'block', fontSize: 14, color: 'var(--grey)', marginTop: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{t.body || 'Sem mensagens'}</span></span>{t.unread > 0 && <span style={{ width: 9, height: 9, borderRadius: 9, background: 'var(--cobalt)', boxShadow: '0 0 0 4px rgba(91,61,245,.16)' }} />}</button>)}</div></div><Nav tab={tab} setTab={setTab} setThread={setThread} setComp={setComp} threads={threads} />{incoming && !activeCall && <IncomingCall call={incoming} onAccept={acceptIncoming} onDecline={declineIncoming} />}{activeCall && <CallOverlay {...activeCall} ping={ping} onClosed={() => setActiveCall(null)} />}</div>;
}
