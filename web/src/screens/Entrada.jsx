    ...
    <div className="lumina-auth" style={{ minHeight: '100dvh', position: 'relative', background: 'linear-gradient(180deg,#EFEDFB,#DFDCF2)' }}>
      ...
      <div className="auth-shell" style={{ position: 'relative', maxWidth: 440, margin: '0 auto', padding: '52px 22px 40px' }}>
        ...
        <p className="up auth-lede" style={{ fontSize: 16.5, lineHeight: 1.42, color: 'var(--grey)', margin: '20px 0 34px', maxWidth: 330, animationDelay: '.12s' }}>
          {t('Feed, Salas, Momentos e conversas num só espaço. Partilha, descobre temas e fala com a tua rede ao teu ritmo.')}
        </p>

        {sent ? (
          <div className="card in auth-card" style={{ padding: 22 }}>
            <h2 className="d" style={{ fontSize: 22, marginBottom: 10 }}>{t('Vê o teu email')}</h2>
            <p style={{ fontSize: 15, lineHeight: 1.45, color: 'var(--grey)' }}>
              {t('Se essa conta existir, enviámos uma ligação para escolheres uma password nova. Expira dentro de uma hora.')}
            </p>
            <button className="p" style={{ marginTop: 18 }} onClick={() => { setSent(false); setMode('login'); }}>{t('Voltar')}</button>
          </div>
        ) : (
          <form onSubmit={submit} className="card in auth-card" style={{ padding: 22, display: 'grid', gap: 12, animationDelay: '.18s' }}>
            {mode === 'registo' && (
              <>
                <input placeholder={t('Como te chamas')} value={f.name} onChange={set('name')} autoComplete="name" required />
                <input placeholder={t('Nome de utilizador')} value={f.handle} onChange={set('handle')}
                  autoCapitalize="none" autoCorrect="off" pattern="[a-z0-9._]{3,24}" required />
                <label className="m" style={{ marginTop: 4 }}>{t('Data de nascimento')}</label>
            ...
            {mode !== 'esqueci' && (
              <input type="password" placeholder={t('Password')} value={f.password} onChange={set('password')}
                autoComplete={mode === 'login' ? 'current-password' : 'new-password'} minLength={8} required />
            )}
            {needsCode && (
              <>
                <label className="m">{t('Código da tua app de autenticação')}</label>
                ...
                <p style={{ fontSize: 12.5, color: 'var(--grey)', lineHeight: 1.4 }}>
                  {t('Perdeste o telemóvel? Usa um dos códigos de emergência que guardaste.')}
                </p>
              </>
            )}

            {mode === 'registo' && (
              <label className="auth-terms" style={{ display: 'flex', gap: 10, alignItems: 'flex-start', fontSize: 13.5, lineHeight: 1.45, color: 'var(--grey)', marginTop: 4 }}>
                <input type="checkbox" checked={terms} onChange={e => setTerms(e.target.checked)}
                  style={{ width: 20, height: 20, flexShrink: 0, marginTop: 1, accentColor: 'var(--coral)' }} required />
                <span>
                  {t('Tenho 16 anos ou mais e aceito os')}{' '}
                  <button type="button" onClick={() => setLegalPage('TERMOS')}
                    style={{ background: 'none', border: 0, padding: 0, font: 'inherit', color: 'var(--cobalt)', cursor: 'pointer', textDecoration: 'underline' }}>{t('termos')}</button>{' '}{t('e a')}{' '}
                  <button type="button" onClick={() => setLegalPage('PRIVACIDADE')}
                    style={{ background: 'none', border: 0, padding: 0, font: 'inherit', color: 'var(--cobalt)', cursor: 'pointer', textDecoration: 'underline' }}>{t('política de privacidade')}</button>.
                </span>
              </label>
            )}
            ...
            <button className="p p-brand" disabled={busy || (mode === 'registo' && !terms)} style={{ padding: 15, fontSize: 15, marginTop: 4 }}>
              {busy ? t('Um momento…') : mode === 'login' ? t('Entrar') : mode === 'registo' ? t('Criar conta') : t('Enviar ligação')}
            </button>
            ...
            <div className="auth-mode-row" style={{ display: 'flex', justifyContent: 'space-between', marginTop: 6 }}>
              <button type="button" className="m" style={{ background: 'none', border: 0, cursor: 'pointer', padding: 0 }}
                ...
                {mode === 'login' ? t('Criar conta') : t('Já tenho conta')}
              </button>
              {mode === 'login' && (
                <button type="button" className="m" style={{ background: 'none', border: 0, cursor: 'pointer', padding: 0 }}
                  onClick={() => { setErr(null); setMode('esqueci'); }}>{t('Esqueci a password')}</button>
              )}
            </div>
          </form>
        )}
