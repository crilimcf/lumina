...
function RoomChat({ room, me, onBack, onRoomUpdated, onRoomDeleted, ping }) {
  ...
  const clearRoomMedia=()=>{setMediaFile(null);setMediaUrl(null);if(roomMediaInput.current)roomMediaInput.current.value=''};
  const send=async()=>{
  ...
  return <div className="lumina-facelift lumina-room-chat" style={{ height:'100dvh',display:'flex',flexDirection:'column',background:'var(--paper)' }}>
    ...
    <div className="room-composer-wrap">
      <input ref={roomMediaInput} type="file" hidden accept="image/jpeg,image/png,image/webp,video/mp4,video/quicktime,video/webm" onChange={e=>{const picked=e.target.files?.[0]||null;e.target.value='';if(picked){setMediaFile(picked);setMediaUrl(null)}}}/>
      {(mediaFile||mediaUrl)&&<div className="room-media-ready"><span>{mediaFile?.type?.startsWith('video/')?'🎥':'📷'}</span><span className="room-media-ready-name">{mediaFile?.name||'Media pronta para enviar'}</span><button type="button" className="messages-media-remove" onClick={clearRoomMedia} aria-label="Remover foto ou vídeo"><X size={15}/></button></div>}
      <div className="room-composer-row"><button type="button" className="room-media-picker" onClick={()=>roomMediaInput.current?.click()} disabled={sending} aria-label="Adicionar foto ou vídeo à sala"><ImagePlus size={18}/></button><input placeholder="Mensagem para a sala…" value={text} onChange={e=>setText(e.target.value)} onKeyDown={e=>e.key==='Enter'&&send()}/><button className="room-send-button" aria-label="Enviar para a sala" onClick={send} disabled={sending||(!text.trim()&&!mediaFile&&!mediaUrl)}>{sending?'…':<Send size={17}/>}</button></div>
    </div>
