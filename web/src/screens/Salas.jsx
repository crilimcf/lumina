...
import { ArrowLeft, AtSign, Camera, CheckCircle2, Crown, DoorOpen, ImagePlus, LockKeyhole, Pencil, Plus, Search, Send, ShieldCheck, Trash2, Users, X } from 'lucide-react';
...
function RoomChat({ room, me, onBack, onRoomUpdated, onRoomDeleted, ping }) {
  ...
  const owner=room.creator_id===me.id;
  const mentionMatch=!privateTarget?text.match(/^@([a-z0-9._]{0,24})$/i):null;
  const mentionQuery=mentionMatch?.[1]||'';
  const pickerOpen=!privateTarget&&(privatePickerOpen||!!mentionMatch);
  ...
  const choosePrivateTarget=(person,body='')=>{setPrivateTarget({id:person.id,name:person.name,handle:person.handle});setPrivatePickerOpen(false);setMentionPeople([]);setText(body);setTimeout(()=>composerInput.current?.focus(),0)};
  const cancelPrivate=()=>{setPrivateTarget(null);setPrivatePickerOpen(false);setMentionPeople([]);setTimeout(()=>composerInput.current?.focus(),0)};
  const togglePrivatePicker=()=>{
    if(privateTarget){cancelPrivate();return;}
    setPrivatePickerOpen(open=>!open);
    if(!privatePickerOpen&&!text.trim())setText('@');
    if(privatePickerOpen&&text==='@')setText('');
    setTimeout(()=>composerInput.current?.focus(),0);
  };
  const changeText=value=>{
    if(!privateTarget){
      if(/^@[a-z0-9._]{0,24}$/i.test(value))setPrivatePickerOpen(true);
      else if(!value.startsWith('@'))setPrivatePickerOpen(false);
      const parsed=value.match(/^@([a-z0-9._]{3,24})\s+(.*)$/i);
      if(parsed){
        const exact=mentionPeople.find(p=>String(p.handle||'').toLowerCase()===parsed[1].toLowerCase());
        if(exact){choosePrivateTarget(exact,parsed[2]);return;}
      }
    }
    setText(value);
  };
  const send=async()=>{
    ...
    try{
      ...
      const wireBody=privateTarget?`@${privateTarget.handle}${body?` ${body}`:''}`:body;
      await api.rooms.send(room.id,wireBody,uploaded);
      setText('');setPrivateTarget(null);setPrivatePickerOpen(false);setMentionPeople([]);setMediaFile(null);setMediaUrl(null);if(roomMediaInput.current)roomMediaInput.current.value='';
  ...
  return <div className="lumina-facelift lumina-room-chat" style={{ height:'100dvh',display:'flex',flexDirection:'column',background:'var(--paper)' }}>
    ...
    <div className="room-messages" style={{ flex:1,overflowY:'auto',padding:14,display:'flex',flexDirection:'column',gap:9 }}>{messages.length===0&&<Empty>A sala está silenciosa.<br/>Diz qualquer coisa.</Empty>}{messages.map(m=>{const video=String(m.media_mime||'').startsWith('video/');const isPrivate=!!m.private_recipient_id;const counterpart=m.sender_id===me.id&&isPrivate?{id:m.private_recipient_id,name:m.private_recipient_name,handle:m.private_recipient_handle}:m.sender_id!==me.id?{id:m.sender_id,name:m.name,handle:m.handle}:null;return <div key={m.id} className={`card room-message${m.sender_id===me.id?' room-message-me':''}${isPrivate?' is-private':''}`} style={{ padding:11,alignSelf:m.sender_id===me.id?'flex-end':'flex-start',maxWidth:'86%',border:isPrivate?'1px solid rgba(85,66,201,.25)':undefined }}><div className="m" style={{ marginBottom:4,display:'flex',alignItems:'center',gap:7,flexWrap:'wrap' }}><span>{m.name||`@${m.handle}`}</span>{isPrivate&&<span className="room-private-badge" style={{ display:'inline-flex',alignItems:'center',gap:4,padding:'3px 7px',borderRadius:999,background:'#F0EEFF',color:'#5542C9',fontSize:9.5,fontWeight:850 }}><LockKeyhole size={10}/>{m.sender_id===me.id?`Privada para @${m.private_recipient_handle||'pessoa'}`:'Privada contigo'}</span>}</div>{m.media_url&&(video?<video className="room-message-media is-video" src={m.media_url} controls playsInline preload="metadata"/>:<img className="room-message-media" src={m.media_url} alt="" loading="lazy"/>)}{m.body&&<div className="room-message-body" style={{ marginTop:m.media_url?8:0 }}>{m.body}</div>}<div style={{ display:'flex',alignItems:'center',gap:10,marginTop:6,flexWrap:'wrap' }}>{counterpart&&<button type="button" onClick={()=>choosePrivateTarget(counterpart)} className="room-private-reply-action" style={{ border:0,background:'transparent',color:'#5542C9',fontSize:10,fontWeight:800,padding:0 }}>{isPrivate?'Continuar em privado':'Responder em privado'}</button>}{(m.sender_id===me.id||(!isPrivate&&owner))&&<button onClick={()=>remove(m)} aria-label="Apagar mensagem" style={{ border:0,background:'transparent',color:'var(--coral)',fontSize:10,padding:0 }}>Apagar</button>}</div></div>})}</div>
    <div className="room-composer-wrap">
      ...
      {privateTarget&&<div className="room-private-target" role="status" style={{ display:'flex',alignItems:'center',gap:8,margin:'0 0 8px',padding:'9px 10px',borderRadius:14,background:'#F0EEFF',color:'#5542C9',fontSize:11.5,fontWeight:750 }}><LockKeyhole size={14}/><span style={{ flex:1,minWidth:0 }}>Só <b>@{privateTarget.handle}</b> verá esta mensagem.</span><button type="button" onClick={cancelPrivate} disabled={sending} aria-label="Cancelar mensagem privada" style={{ border:0,background:'transparent',color:'inherit',display:'grid',placeItems:'center',padding:2 }}><X size={14}/></button></div>}
      {pickerOpen&&<div className="room-private-picker"><div className="m room-private-picker-title">Enviar em privado para</div>{mentionPeople.length===0?<div className="room-private-picker-empty">Não há outra pessoa correspondente nesta Sala.</div>:mentionPeople.map(p=><button key={p.id} type="button" className="room-private-person" onClick={()=>choosePrivateTarget(p)}><span className="room-private-person-avatar">{String(p.name||p.handle||'?').slice(0,1).toUpperCase()}</span><span className="room-private-person-copy"><b>{p.name}</b><small>@{p.handle}</small></span><LockKeyhole size={14}/></button>)}</div>}
      ...
      <div className="room-composer-row"><button type="button" className={`room-private-picker-button${pickerOpen?' is-active':''}`} onClick={togglePrivatePicker} disabled={sending} aria-label="Escolher pessoa para mensagem privada" aria-pressed={pickerOpen}><AtSign size={18}/></button><button type="button" className="room-media-picker" onClick={()=>roomMediaInput.current?.click()} disabled={sending} aria-label="Adicionar foto ou vídeo à sala"><ImagePlus size={18}/></button><input ref={composerInput} placeholder={privateTarget?`Mensagem privada para @${privateTarget.handle}…`:'Mensagem para a sala…'} value={text} onChange={e=>changeText(e.target.value)} onKeyDown={e=>e.key==='Enter'&&send()} disabled={sending}/><button className="room-send-button" aria-label={privateTarget?`Enviar em privado para @${privateTarget.handle}`:'Enviar para a sala'} onClick={send} disabled={sending||(!text.trim()&&!mediaFile&&!mediaUrl)}>{sending?'…':<Send size={17}/>}</button></div>
      {!privateTarget&&<div className="m room-private-hint">Toca em <b>@</b> ou começa por <b>@utilizador</b> para falar só com uma pessoa desta Sala.</div>}
    </div>
