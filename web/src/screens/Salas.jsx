...
import { ArrowLeft, Camera, Crown, DoorOpen, LockKeyhole, Pencil, Plus, Search, Send, ShieldCheck, Trash2, Users, X } from 'lucide-react';
...
function RoomChat({ room, me, onBack, onRoomUpdated, onRoomDeleted, ping }) {
  ...
  useEffect(()=>{setEditName(room.name||'');setEditTopic(room.topic||'');setEditDescription(room.description||'')},[room.id,room.name,room.topic,room.description]);
  ...
  const send=async()=>{if(!text.trim())return;const body=text.trim();setText('');try{await api.rooms.send(room.id,body);setMessages(await api.rooms.messages(room.id))}catch(e){setText(body);ping(e.message)}};
  ...
  const saveRoom=async()=>{const name=editName.trim();const topic=editTopic.trim();if(name.length<3||topic.length<3)return ping('Dá um nome e um tópico à sala');setSaving(true);try{const updated=await api.rooms.update(room.id,{name,topic,description:editDescription.trim()});onRoomUpdated(updated);setShowEdit(false);ping('Sala atualizada')}catch(e){ping(e.message)}finally{setSaving(false)}};
  ...
  return <div style={{ height:'100dvh',display:'flex',flexDirection:'column',background:'var(--paper)' }}>
    <header style={{ padding:'12px 14px',display:'flex',alignItems:'center',gap:8,borderBottom:'1px solid #E1DDF0' }}><button className="p" onClick={onBack} aria-label="Voltar às salas"><ArrowLeft size={16}/></button><div style={{ width:42,height:42,borderRadius:14,overflow:'hidden',background:'#25154F',flexShrink:0 }}>{room.image_url&&<img src={room.image_url} alt="" style={{ width:'100%',height:'100%',objectFit:'cover',display:'block' }}/>}</div><div style={{ flex:1,minWidth:0 }}><b style={{ display:'block',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis' }}>{room.name}</b><span className="m" style={{ display:'block',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis' }}>{room.topic}</span></div>{owner&&<button className="p" onClick={()=>setShowEdit(true)} aria-label="Editar sala"><Pencil size={16}/></button>}{owner&&room.visibility!=='public'&&<button className="p" onClick={()=>setShowInvite(v=>!v)} aria-label="Convidar pessoas"><Plus size={17}/></button>}{owner&&<button className="p" onClick={()=>setShowDelete(true)} aria-label="Apagar sala"><Trash2 size={16}/></button>}</header>
    {showInvite&&<div style={{ padding:12,borderBottom:'1px solid #E1DDF0' }}><div style={{ position:'relative' }}><Search size={15} style={{ position:'absolute',left:11,top:12 }}/><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Procurar utilizador" style={{ paddingLeft:34 }}/></div>{people.map(p=><button key={p.id} className="p" onClick={async()=>{try{await api.rooms.invite(room.id,p.id);ping(`Convite enviado a ${p.name}`);setSearch('');setPeople([])}catch(e){ping(e.message)}}} style={{ width:'100%',marginTop:7,justifyContent:'space-between' }}><span>{p.name} · @{p.handle}</span><Plus size={14}/></button>)}</div>}
    <div style={{ flex:1,overflowY:'auto',padding:14,display:'flex',flexDirection:'column',gap:9 }}>{messages.length===0&&<Empty>A sala está silenciosa.<br/>Diz qualquer coisa.</Empty>}{messages.map(m=><div key={m.id} className="card" style={{ padding:11,alignSelf:m.sender_id===me.id?'flex-end':'flex-start',maxWidth:'86%' }}><div className="m" style={{ marginBottom:4 }}>@{m.sender_handle}</div><div>{m.body}</div>{(m.sender_id===me.id||owner)&&<button onClick={()=>remove(m)} aria-label="Apagar mensagem" style={{ border:0,background:'transparent',color:'var(--coral)',fontSize:10,marginTop:6,padding:0 }}>Apagar</button>}</div>)}</div>
    <div style={{ padding:'10px 12px calc(12px + env(safe-area-inset-bottom))',display:'flex',gap:8,borderTop:'1px solid #E1DDF0' }}><input placeholder="Mensagem para a sala…" value={text} onChange={e=>setText(e.target.value)} onKeyDown={e=>e.key==='Enter'&&send()}/><button className="p p-brand" aria-label="Enviar para a sala" onClick={send}><Send size={16}/></button></div>
    {showEdit&&<div style={{ position:'fixed',inset:0,zIndex:120,background:'rgba(15,10,35,.52)',display:'grid',placeItems:'center',padding:18 }}><div className="card" style={{ width:'100%',maxWidth:420,padding:18 }}><div className="d" style={{ fontSize:25,marginBottom:12 }}>Editar sala</div><input placeholder="Nome da sala" value={editName} onChange={e=>setEditName(e.target.value)} style={{ marginBottom:9 }}/><input placeholder="Tópico principal" value={editTopic} onChange={e=>setEditTopic(e.target.value)} style={{ marginBottom:9 }}/><textarea placeholder="Descrição (opcional)" value={editDescription} onChange={e=>setEditDescription(e.target.value)} rows={3} style={{ width:'100%',resize:'none' }}/><div style={{ display:'flex',gap:8,marginTop:12 }}><button className="p" onClick={()=>setShowEdit(false)}>Cancelar</button><button className="p p-brand" onClick={saveRoom} disabled={saving}>Guardar alterações</button></div></div></div>}
    {showDelete&&<div role="dialog" aria-label="Confirmar apagar sala" style={{ position:'fixed',inset:0,zIndex:125,background:'rgba(15,10,35,.58)',display:'grid',placeItems:'center',padding:18 }}><div className="card" style={{ width:'100%',maxWidth:380,padding:18 }}><div className="d" style={{ fontSize:25 }}>Apagar sala?</div><div className="m" style={{ margin:'8px 0 15px' }}>As mensagens desta sala também serão removidas.</div><div style={{ display:'flex',gap:8 }}><button className="p" onClick={()=>setShowDelete(false)}>Cancelar</button><button className="p" onClick={deleteRoom} disabled={saving} style={{ color:'var(--coral)' }}>Apagar sala</button></div></div></div>}
  </div>;
}
...
export function Salas({ me,tab,setTab,setComp,threads,setThread,ping,toast,unreadCount }) {
  ...
  return <div style={{ minHeight:'100dvh',paddingBottom:100 }}><div style={{ maxWidth:620,margin:'0 auto',padding:'20px 16px' }}><div style={{ display:'flex',alignItems:'center',gap:8,margin:'4px 0 10px' }}><div style={{ flex:1 }}><h2 className="d" style={{ fontSize:40,margin:0 }}>Sa<span className="it">las</span></h2><div className="m" style={{ marginTop:4 }}>Tópicos vivos, sem poluir o Feed.</div></div><button className="p p-brand p-sm" onClick={()=>setCreating(true)}><Plus size={15}/> Criar</button><TopActions tab={tab} setTab={setTab} setThread={setThread} unreadCount={unreadCount}/></div><div style={{ display:'flex',gap:7,overflowX:'auto',padding:'9px 0 15px' }}>{roomFilters.map(([k,l])=><button key={k} onClick={()=>setFilter(k)} className={`p p-sm${filter===k?' p-ink':''}`} style={{ flexShrink:0 }}>{l}</button>)}</div>{visible.length===0?<Empty>Não há salas nesta categoria.<br/>Cria a primeira.</Empty>:<div style={{ display:'grid',gap:13 }}>{visible.map(room=><RoomCard key={room.id} room={room} me={me} onOpen={setActive} onRefresh={load} ping={ping}/>)}</div>}<div className="card" style={{ padding:14,marginTop:16,display:'flex',gap:10,alignItems:'center' }}><ShieldCheck size={20} color="var(--cobalt)"/><div style={{ fontSize:12.5,lineHeight:1.4 }}><b>Privacidade real.</b> Salas privadas só aparecem a pessoas convidadas por quem as criou.</div></div></div><Nav tab={tab} setTab={setTab} setThread={setThread} setComp={setComp} threads={threads}/><Toast text={toast}/>{creating&&<CreateRoom onClose={()=>setCreating(false)} onCreated={room=>setRooms(current=>[room,...current])} ping={ping}/>}</div>;
}
