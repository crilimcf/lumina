...
function RoomChat({ room, me, onBack, onRoomUpdated, onRoomDeleted, ping }) {
  ...
  const owner=room.creator_id===me.id;
  const mentionQuery=!privateTarget?(text.match(/^@([a-z0-9._]{2,24})$/i)?.[1]||''):'';
  ...
  const cancelPrivate=()=>{setPrivateTarget(null);setMentionPeople([]);setTimeout(()=>composerInput.current?.focus(),0)};
  ...
  const send=async()=>{
    ...
    try{
      ...
      setText('');setPrivateTarget(null);setMentionPeople([]);setMediaFile(null);setMediaUrl(null);if(roomMediaInput.current)roomMediaInput.current.value='';
  ...
  return <div className="lumina-facelift lumina-room-chat" style={{ height:'100dvh',display:'flex',flexDirection:'column',background:'var(--paper)' }}>
    ...
    <div className="room-messages" style={{ flex:1,overflowY:'auto',padding:14,display:'flex',flexDirection:'column',gap:9 }}>{messages.length===0&&<Empty>A sala está silenciosa.<br/>Diz qualquer coisa.</Empty>}{messages.map(m=>{const video=String(m.media_mime||'').startsWith('video/');const isPrivate=!!m.private_recipient_id;const counterpart=m.sender_id===me.id&&isPrivate?{id:m.private_recipient_id,name:m.private_recipient_name,handle:m.private_recipient_handle}:m.sender_id!==me.id?{id:m.sender_id,name:m.name,handle:m.handle}:null;return <div key={m.id} className={`card room-message${m.sender_id===me.id?' room-message-me':''}`} style={{ padding:11,alignSelf:m.sender_id===me.id?'flex-end':'flex-start',maxWidth:'86%',border:isPrivate?'1px solid rgba(85,66,201,.25)':undefined }}><div className="m" style={{ marginBottom:4,display:'flex',alignItems:'center',gap:7,flexWrap:'wrap' }}><span>{m.name||`@${m.handle}`}</span>{isPrivate&&<span style={{ display:'inline-flex',alignItems:'center',gap:4,padding:'3px 7px',borderRadius:999,background:'#F0EEFF',color:'#5542C9',fontSize:9.5,fontWeight:850 }}><LockKeyhole size={10}/>{m.sender_id===me.id?`Privada para @${m.private_recipient_handle||'pessoa'}`:'Privada contigo'}</span>}</div>{m.media_url&&(video?<video className="room-message-media is-video" src={m.media_url} controls playsInline preload="metadata"/>:<img className="room-message-media" src={m.media_url} alt="" loading="lazy"/>)}{m.body&&<div style={{ marginTop:m.media_url?8:0 }}>{m.body}</div>}<div style={{ display:'flex',alignItems:'center',gap:10,marginTop:6,flexWrap:'wrap' }}>{counterpart&&<button type="button" onClick={()=>choosePrivateTarget(counterpart)} style={{ border:0,background:'transparent',color:'#5542C9',fontSize:10,fontWeight:800,padding:0 }}>{isPrivate?'Continuar em privado':'Responder em privado'}</button>}{(m.sender_id===me.id||(!isPrivate&&owner))&&<button onClick={()=>remove(m)} aria-label="Apagar mensagem" style={{ border:0,background:'transparent',color:'var(--coral)',fontSize:10,padding:0 }}>Apagar</button>}</div></div>})}</div>
    <div className="room-composer-wrap">
      ...
      {privateTarget&&<div role="status" style={{ display:'flex',alignItems:'center',gap:8,margin:'0 0 8px',padding:'9px 10px',borderRadius:14,background:'#F0EEFF',color:'#5542C9',fontSize:11.5,fontWeight:750 }}><LockKeyhole size={14}/><span style={{ flex:1,minWidth:0 }}>Só <b>@{privateTarget.handle}</b> verá esta mensagem.</span><button type="button" onClick={cancelPrivate} disabled={sending} aria-label="Cancelar mensagem privada" style={{ border:0,background:'transparent',color:'inherit',display:'grid',placeItems:'center',padding:2 }}><X size={14}/></button></div>}
      {!privateTarget&&mentionPeople.length>0&&<div style={{ margin:'0 0 8px',padding:6,borderRadius:15,background:'#fff',border:'1px solid #E1DDF0',boxShadow:'0 8px 24px rgba(35,24,78,.08)' }}><div className="m" style={{ padding:'5px 7px 4px' }}>Enviar só para uma pessoa</div>{mentionPeople.map(p=><button key={p.id} type="button" onClick={()=>choosePrivateTarget(p)} style={{ width:'100%',border:0,background:'transparent',display:'flex',alignItems:'center',justifyContent:'space-between',gap:8,padding:'8px 9px',borderRadius:11,textAlign:'left',color:'inherit' }}><span><b>{p.name}</b> <span className="m">@{p.handle}</span></span><LockKeyhole size={13}/></button>)}</div>}
      ...
      {!privateTarget&&<div className="m" style={{ marginTop:6,padding:'0 4px',fontSize:10.5 }}>Começa com <b>@utilizador</b> para falar só com essa pessoa dentro da sala.</div>}
    </div>
